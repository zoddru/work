...
test('total', t => {
    const pm = PayModel.create({ payPoints: [{ money: 2230, staff: 3 }, { money: 4300, staff: 2 }] });
    t.is(pm.total, 15290);
});
...
test('next total no percentage increase', t => {
    const pm = PayModel.create({ payPoints: [{ money: 2201, staff: 3 }, { money: 3301, staff: 2 }] });
    t.is(pm.nextTotal, 13205);
});

test('next total with percentage increase', t => {
    const payPoints = [{ money: 2000, staff: 3 }, { money: 3000, staff: 2 }, { money: 4000, staff: 1 }];
    let pm = PayModel.create({ payPoints });
    t.is(pm.nextTotal, 16000);

    pm = PayModel.create({ payPoints, percentageIncrease: 1 });
    t.is(pm.nextTotal, 16160);

    pm = PayModel.create({ payPoints, percentageIncrease: 1.5 });
    t.true(th.isClose(pm.nextTotal, 16240));

    pm = PayModel.create({ payPoints, percentageIncrease: 2 });
    t.is(pm.nextTotal, 16320);
});

test('difference', t => {
    const pm = PayModel.create({ payPoints: [{ money: 2000, staff: 3 }, { money: 3000, staff: 2 }, { money: 4000, staff: 1 }], percentageIncrease: 1.5 });
    t.true(th.isClose(pm.nextTotal, 16240));
    t.true(th.isClose(pm.difference, 240));
});

test('change percentageIncrease', t => {
    let pm = PayModel.create({ payPoints: [{ money: 2000, staff: 3 }, { money: 3000, staff: 2 }, { money: 4000, staff: 1 }] });
...
test('change pay points', t => {
    let pm = PayModel.create({ payPoints: [{ money: 2000, staff: 3 }, { money: 3000, staff: 2 }, { money: 4000, staff: 1 }], percentageIncrease: 1.5 });
