...
test('create defaults', t => {
    ...
    t.is(pp.money, 0);
    t.is(pp.staff, 0);
    t.is(pp.increase.value, 1);
});

test('create', t => {
    const pp = new PayPoint({ index: 1, name: 'm1', money: 1234, staff: 3, increase: { value: 2 } });
...
test('calculates nextMoney', t => {
    let pp = new PayPoint({ money: 1234, staff: 3, increase: { value: 2 } });
    t.is(pp.nextMoney, 2468);

    pp = new PayPoint({ money: 1000, staff: 3, increase: { value: 1.5 } });
    t.is(pp.nextMoney, 1500);

    pp = new PayPoint({ money: 3333, staff: 2, increase: { value: 1 } });
...
test('calculates nextMoney when increase changed', t => {
    ...
    const pp = new PayPoint({ money: 1000, staff: 3, increase });
...
test('calculates nextTotal when increase changed', t => {
    ...
    const pp = new PayPoint({ money: 1500, staff: 3, increase });
    t.is(pp.nextTotal, 4500);
